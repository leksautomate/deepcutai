...
export async function registerRoutes(
  ...
  app.post("/api/settings/api-keys", async (req, res) => {
    try {
      ...
      const { gemini, groq, speechify, freepik, wavespeed, runpod, pollinations, inworld } = req.body;
  ...
  app.post("/api/generate-assets", async (req, res) => {
    try {
      const { script, voiceId, imageStyle, customStyleText, resolution, motionEffect, imageGenerator = "seedream", pollinationsModel = "flux", ttsProvider = "speechify", sceneSettings } = req.body;
      ...
      for (let i = 0; i < scenes.length; i++) {
        ...
        if (imageGenerator === "pollinations") {
          ...
          try {
            ...
            // Write the image buffer directly to file
            fs.writeFileSync(imagePath, result.imageBuffer);
            
            imageResult = { success: true };
          } catch (error: any) {
            logError("ASSETS", `Pollinations generation error`, error);
  ...
  app.post("/api/generate-background", async (req, res) => {
    try {
      const { script, title, voiceId, imageStyle, customStyleText, resolution, transition, imageGenerator, pollinationsModel, ttsProvider } = req.body;
  ...
  app.post("/api/generate-image", async (req, res) => {
    try {
      ...
      // Pollinations API key is optional
      ...
      if (generator === "pollinations") {
        ...
        const result = await generateImageWithPollinations(
          prompt,
          apiKey, // Can be null for pollinations
          width || 1024,
          height || 576,
