...
export async function generateInworldTTS(options: InworldTTSOptions): Promise<InworldTTSResult> {
  ...
  if (!apiKey) {
    console.warn("INWORLD_API_KEY not configured, using estimated duration only");
    return {
      audioPath: "",
  ...
  try {
    ...
    if (!data.audioContent) {
      console.error("No audioContent in Inworld TTS response");
      return {
        audioPath: "",
        durationSeconds: estimatedDuration,
        success: false,
    ...
    if (audioBuffer.length === 0) {
      console.error("Empty audio content from Inworld TTS");
      return {
        audioPath: "",
        durationSeconds: estimatedDuration,
        success: false,
    ...
    console.error("Inworld TTS generation error:", error);
    return {
      audioPath: "",
      durationSeconds: estimatedDuration,
      success: false,
