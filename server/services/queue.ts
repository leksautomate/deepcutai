...
import { resolutionOptions, motionEffects, transitionEffects, type VideoManifest, type Scene } from "@shared/schema";
...
async function processScriptProject(project: ScriptQueuedProject) {
  ...
  try {
    ...
    const imageGenerator = project.imageGenerator || "seedream";
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      if ((project as any).ttsProvider === "inworld") {
...
async function processProject(project: QueuedProject) {
  ...
  try {
    ...
    const scriptResult = await generateScript({
      ...
      duration: project.duration as any,
    });
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      if (imageGenerator === "pollinations") {
        ...
        try {
          const apiKey = await getResolvedApiKey("pollinations");
          const pollinationsModel = (project as any).pollinationsModel || "flux";
