...
/**
 * Generates a scene image with character/style reference consistency.
 * Creates a fresh Whisk project per call (stateless, matching Bulkmass approach),
 * adds the raw base64 references, generates, then deletes the project.
 *
 * @param prompt           - Scene image prompt
 * @param cookie           - Google Whisk cookie
 * @param refs             - Raw base64 strings (NO data: prefix) for subject and/or style
 * @param whiskAspectRatio - Whisk aspect ratio constant
 */
export async function generateImageWithWhiskCharacter(
  prompt: string,
  cookie: string,
  refs: { subjectRawBase64?: string; styleRawBase64?: string },
  ...
  return withRetry(
    async () => {
      ...
      if (refs.subjectRawBase64) await project.addSubject({ base64: refs.subjectRawBase64 });
      if (refs.styleRawBase64)   await project.addStyle({ base64: refs.styleRawBase64 });

      // GEM_PIX's userInstruction field works best with plain prose,
      // not the structured markdown (**Style:**, **Subject:**) that Groq generates.
      // Strip the headers and collapse to a single flowing description.
      const cleanPrompt = prompt
        .replace(/\*\*[^*]+:\*\*\s*/g, "")   // remove **Header:** markers
        .replace(/\n{2,}/g, " ")              // collapse paragraph breaks
        .trim();
      const finalPrompt = cleanPrompt + ", NO TEXT, NO WATERMARK, clear image";
      ...
      if (!media) throw new Error("Whisk character generation returned no image data");

      logInfo("WhiskCharacter", "Character-consistent image generated", { aspectRatio: whiskAspectRatio });

      let resultData: string = media.encodedMedia;
      if (!resultData.startsWith("data:")) resultData = `data:image/png;base64,${resultData}`;
      return resultData;
    },
    {
      maxRetries: 4,
